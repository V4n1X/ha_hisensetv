...
| Property | Value |
|---|---|
| Transport | MQTT 3.1.1 (Eclipse Paho in the app), broker **runs on the TV** |
| Port | **36669** (advertised per device as `mqttport=`) |
| TLS | `ssl://` when the advertised `transport_protocol >= 1001` (APK `MqttConnectManager`: `i2 >= 1001`); self-signed server cert → verify off / TOFU |
...
| `/authentication` | – | **TV requests pairing** (triggers PIN display) |
| `/authenticationcode` | `{result: 0|1, info}` | result of a PIN submission |
| `/authenticationcodetoast` | – | success confirmation variant |
| `/authenticationcodeclose` | – | pairing dialog dismissed |
...
TV unknown? ──► push function "/authentication"
             ──► app shows PIN entry; TV shows 4-digit code
user enters ──► publish {"authNum":"NNNN"} to ui_service/<cid>/actions/authenticationcode
TV checks   ──► push "/authenticationcode" {"result":1}        -> done
                push "/authenticationcodetoast"               -> done (toast)
                push {"result":0}                             -> retry
